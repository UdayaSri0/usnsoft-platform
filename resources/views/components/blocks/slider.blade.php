    ...
    <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        @foreach ($slides as $slide)
            <article class="usn-card">
                <h3 class="font-display text-xl font-semibold text-slate-950 dark:text-slate-50">{{ $slide['title'] ?? 'Slide' }}</h3>
                @if (!empty($slide['subtitle']))
                    <p class="mt-2 text-sm font-medium text-sky-700 dark:text-sky-300">{{ $slide['subtitle'] }}</p>
                @endif
                @if (!empty($slide['body']))
                    <p class="mt-4 text-sm leading-6 text-slate-600 dark:text-slate-300">{{ $slide['body'] }}</p>
                @endif
