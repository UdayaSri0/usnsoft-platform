<x-app-layout>
    <x-slot name="header">
        <x-ui.page-header title="FAQ Categories" description="Group public answers into clear sections." eyebrow="Content" />
    </x-slot>
    ...
    <div class="py-8">
        <div class="usn-container space-y-6">
            ...
            <section class="usn-card">
                <form method="POST" action="{{ route('admin.faq.categories.store') }}" class="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
                    @csrf
            ...
            <section class="usn-card space-y-4">
                @foreach ($categories as $category)
                    <form method="POST" action="{{ route('admin.faq.categories.update', ['category' => $category->getKey()]) }}" class="grid gap-4 border-b border-slate-200 pb-4 last:border-0 last:pb-0 md:grid-cols-2 xl:grid-cols-6">
                        @csrf
                        @method('PUT')
